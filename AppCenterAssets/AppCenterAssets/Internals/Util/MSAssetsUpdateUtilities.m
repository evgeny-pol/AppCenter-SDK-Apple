...
- (NSString *)getHashForBinaryContents:(NSURL *)binaryBundleUrl
    ...
    NSMutableDictionary *binaryHashDictionary = [[[self assetsDeploymentInstance] settingManager] getBinaryHash];
    NSString *binaryHash = nil;
    if (binaryHashDictionary != nil) {
        binaryHash = [binaryHashDictionary objectForKey:binaryModifiedDate];
        if (binaryHash == nil) {
            [[[self assetsDeploymentInstance] settingManager] removeBinaryHash];
        } else {
            return binaryHash;
        }
    }
    ...
    [[[self assetsDeploymentInstance] settingManager] saveBinaryHash:binaryHashDictionary];
    return binaryHash;
}
